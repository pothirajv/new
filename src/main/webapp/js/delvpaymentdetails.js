...
$(document).ready(function(){
	...
	$("#userNameSrchSel").change(function() {
		...
		$.ajax({
			url : $("#contextPath").val()+'/adm/getBillingCycleToPaySettlementToDeliverer?userId=' + userId + '&pendingFlag=' + pendingFlag + '&userRole=' + userRole,
			type : 'POST',
				...
				if(data.length > 0) {
					...
					$.each(data,function(i,val){
						...
						if(val.payCompleteFlag != true) {
							addTxt = ' (Pending)';
							opt += '<option cflg="false" style="background: #ffffff;" fromDate="'+val.fromDate+'" toDate="'+val.toDate+'" billId="'+val.id+'" userId="'+userId+'" billidx="'+i+'" compflag="'+val.payCompleteFlag+'" value="'+val.id+'">'+billingCycleVar + addTxt + '</option>';
						} else {
							addTxt = ' (Completed)';
							opt += '<option cflg="true" style="background: #ffffff;" fromDate="'+val.fromDate+'" toDate="'+val.toDate+'" billId="'+val.id+'" userId="'+userId+'" billidx="'+i+'" compflag="'+val.payCompleteFlag+'" value="'+val.id+'">'+billingCycleVar + addTxt + '</option>';
						}
					});
...
//			alert('Please Select Billing Cycle');
	...
	$(document.body).on('click', '.generateExcel', function() {
		if(searchData != null && searchData != undefined && searchData.length > 0) {
			...
			} else if(userRole == 'ROLE_DELIVER' || userRole == 'ROLE_ADMIN') {
				if(employeeFlag == 'true' || userRole == 'ROLE_ADMIN') {
...
function savePaymentDetails() {
	...
	paymentDetailsDto.receivedUserId = $('#userNameSrchSel').val();
...
function loadVmanagementDataTbl(data, userRole){
	...
	var totChargeHdr = 0;
	var totCmmsnAllowHdr = 0;
	var totRiderTravelDistHdr = 0;
	if(data != null && data != undefined){
		...
		for (var i = 0; i < data.length; i++) {
			...
			totCmmsnAllowHdr += parseFloat(taxTblDataObj.riderCommission) + parseFloat(taxTblDataObj.riderTravelAllowance);
			totRiderTravelDistHdr += parseFloat(taxTblDataObj.riderTravelDistance);
			
			if(taxTblDataObj.senderRequestModel.assignedUserModel.employee == true) {
				totAmntHdr = totRiderTravelDistHdr * taxTblDataObj.riderAllowancePerKm;
			} else {
				totAmntHdr = totDelvChargeHdr * (taxTblDataObj.riderCmsnPerc / 100);
			}
		}
	}
	totCmmsnAllowHdr = Math.ceil(totCmmsnAllowHdr);
	...
	$('#totCmmsnAllowHdr').html(Number.isInteger(totCmmsnAllowHdr)? totCmmsnAllowHdr : totCmmsnAllowHdr.toFixed(2));
	$('#totRiderTravelDistHdr').html(Number.isInteger(totRiderTravelDistHdr)? totRiderTravelDistHdr : totRiderTravelDistHdr.toFixed(2));
	
	if(taxTblDataObj.senderRequestModel.assignedUserModel.employee == true) {
		$('.totCmmsnAllowLbl').html('Total Travel Allowance');
	} else {
		$('.totCmmsnAllowLbl').html('Total Rider Commission');
	}
